...
class TD3LearnerCore(learner_core_lib.LearnerCore):
    """TD3 learner."""
    ...
    def __init__(
        ...
        def make_initial_state(random_key):
            (
                key_init_policy,
                key_init_twin,
                key_init_target,
        ...
        self._init_state = make_initial_state

    def init(self, random_key: networks_lib.PRNGKey):
        return self._init_state(random_key)

    def step(self, state: TrainingState, transitions: types.Transition):
        # Get data from replay (dropping extras if any). Note there is no
        # extra data here because we do not insert any into Reverb.

        return self._update_step(state, transitions)

    def get_variables(
        self, state: TrainingState, names: List[str]
    ) -> List[networks_lib.Params]:
        variables = {
            "policy": state.policy_params,
            "critic": state.critic_params,
            "twin_critic": state.twin_critic_params,
        }
        return [variables[name] for name in names]


# TODO(yl): Consider removing this
class TD3Learner(learners.DefaultJaxLearner):
    """TD3 learner."""
    ...
    def __init__(
        ...
        super().__init__(
            core, random_key, iterator=iterator, logger=logger, counter=counter
        )
