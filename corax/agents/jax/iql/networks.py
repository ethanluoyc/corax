...
def _default_init(scale: float = onp.sqrt(2)):
...
class Policy(hk.Module):
    ...
    def __call__(
        ...
        means = hk.Linear(self.action_dim, w_init=_default_init())(outputs)  # type: ignore
...
class ValueCritic(hk.Module):
    ...
    def __call__(self, observations: jnp.ndarray) -> jnp.ndarray:
        critic_value = hk.nets.MLP(
            output_sizes=(*self.hidden_dims, 1),  # type: ignore
            w_init=_default_init(),
        )(observations)
...
class DoubleCritic(hk.Module):
    ...
    def __call__(
        ...
        critic1 = Critic(self.hidden_dims, activations=self.activations)(  # type: ignore
            observations, actions
        )
        critic2 = Critic(self.hidden_dims, activations=self.activations)(  # type: ignore
            observations, actions
        )
