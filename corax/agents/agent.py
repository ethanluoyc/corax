...
class Agent(core.Actor, core.VariableSource):
    ...
    def __init__(
        ...
        self._batch_size_upper_bounds = (
            [1_000_000_000] * len(replay_tables) if replay_tables else None
        )
    ...
    def update(
        ...
        if self._iterator:
            ...
            while self._has_data_for_training():
                ...
                self._batch_size_upper_bounds = [
                    math.ceil(
                        t.info.rate_limiter_info.sample_stats.completed
                        / (total_batches + 1)
                    )
                    for t in self._replay_tables  # type: ignore
                ]
                update_actor = True
            if update_actor:
        ...
        num_steps = _calculate_num_learner_steps(
            ...
            min_observations=self._min_observations,  # type: ignore
            observations_per_step=self._observations_per_step,  # type: ignore
        )
