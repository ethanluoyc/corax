...
def update(
    ...
    *,
    ...
    def compute_std(
        path: Path, summed_variance: jnp.ndarray, std: jnp.ndarray
    ) -> jnp.ndarray:
    ...
    std = tree_utils.fast_map_structure_with_path(
        compute_std, summed_variance, state.std
    )
...
def get_clip_config_for_path(
    config: NestClippingConfig, path: Path
) -> NestClippingConfig:
...
def clip(
    batch: types.NestedArray, clipping_config: NestClippingConfig
) -> types.NestedArray:
    """Clips the batch."""
