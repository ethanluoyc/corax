...
try:
    ...
    from corax.wrappers import gymnasium_wrapper
...
class GymWrapperTest(absltest.TestCase):
    ...
    def test_early_truncation(self):
        ...
        try:
            gym_env = gymnasium.make("Pendulum-v1")
        except:  # noqa
            gym_env = gymnasium.make("Pendulum-v0")
        env = gymnasium_wrapper.GymnasiumWrapper(gym_env)
        ts = env.reset()
        while not ts.last():
