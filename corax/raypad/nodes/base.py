...
from corax.raypad import address as raypad_address
from corax.raypad import context as rp_context
...
class Node(Generic[HandleType], metaclass=abc.ABCMeta):
    ...
    def __init__(self) -> None:
        ...
        # launcher.
        self._launch_context = rp_context.LaunchContext()
        ...
        # mechanism.
        self.addresses = []  # type: List[raypad_address.Address]

    @property
    def launch_context(self):
        return self._launch_context

    def _initialize_context(self, launch_type, launch_config: Any):
    ...
    def allocate_address(self, address: raypad_address.Address) -> None:
    ...
    @classmethod
    def default_launch_config(cls, launch_type: rp_context.LaunchType):
