...
class Address(object):
    ...
    def __init__(self, name: Optional[str] = None):
        ...
        if name is not None and not _ADDRESS_NAME_VALID_PATTERN.fullmatch(name):
            raise ValueError(
                f"Wrong address name: {name} does not match "
        ...
        if self._owning_node is not None:
            if self._owning_node is node:
                logging.warning(
                    ...
                    "of type %s it's probably a mistake.",
                    node,
                )
                return
            raise ValueError(
                "You are associating a node to this lp.Address which "
    ...
    def __getstate__(self):
        ...
        if not isinstance(self._owning_node, str):
            state["_owning_node"] = "Restored from pickle node named: " + repr(
                self._owning_node
            )
        return state
